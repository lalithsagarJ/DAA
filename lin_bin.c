...
void display(int ele,int i,int st,int et,int t){
	printf("\nelement %d is present in position %d",ele,i);
	printf("\nstart time=%d\tend time=%d\ntotal time=%d\n",st,et,t);
}

int lin_ite(int n,int a[50]){
	int ele,i,st,p,et,t_t;
...
a:	et-clock();
	t=-et-st;
	if(loc>-1)
		display(ele,loc+1,st,et,t);
	return loc;
}
int bin_ite(int n,int a[50]){
	int st,et,t,ele,beg=0,end=n-1;
	printf("\nenter the search element:");
	scanf("%d",&ele);
	st=clock();
	while (beg<=end){
